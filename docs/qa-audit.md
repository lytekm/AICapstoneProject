...
| Persona and length appear inert in mock mode | The mock abstractor is deterministic and ignores persona styling and length scaling by design. | During offline/mock runs, the controls do not demonstrate the production LLM behavior. | Document this as a known limitation rather than pretending the mock simulates the real model. | Known limitation | n/a |
...
| Persona and length appear inert in mock mode | The mock abstractor is deterministic and ignores persona styling and length scaling by design. | Offline/mock runs do not demonstrate production LLM behavior. | Documented as an explicit limitation of mock mode. | Known limitation | n/a |
