...
The second pass tightened runtime semantics after live testing against the real vLLM path. These items were implemented in `80b621e`.

| Symptom | Root cause | Impact | Fix / disposition | Status | Commit |
| --- | --- | --- | --- | --- | --- |
| Real vLLM requests could fail even with `USE_MOCK_LLM=0` | The runtime left the model name at `"default"` unless a caller injected a config object directly. | The app could point at a healthy vLLM endpoint and still fail requests because the model name was invalid for the server. | `AbstractorConfig` now reads `VLLM_MODEL` from the environment, and the Kubernetes manifest supplies the concrete model name. | Fixed | `80b621e` |
| Abstractive summaries always rendered `N/A` confidence | Verification only ran in hybrid mode. | Users could not tell whether an abstractive summary introduced unsupported entities. | Abstractive mode now uses the same verifier path as hybrid when verification is available, and still returns `null` when it is not. | Fixed | `80b621e` |
| Extractive length selector had no effect on output length | Extractive mode ignored `length` entirely and only used the raw `k` value. | The UI exposed a control that appeared inert in one of the three modes. | Extractive mode now scales the effective sentence budget from `k` using the selected length profile. | Fixed | `80b621e` |
| Verification over-flagged dates, list numbering, and citation scaffolding | The verifier compared raw spaCy entities without filtering noisy entity classes or formatting artifacts. | Confidence scores looked harsher than the actual grounding problem, especially for executive and academic personas. | The verifier now strips reference scaffolding, ignores numeric/date entity classes, and normalizes markdown/list formatting before NER comparison. | Fixed | `80b621e` |

## Verification Metric Semantics

- `confidence` is a grounding signal, not a direct quality score. Higher means more of the summary's kept named entities were also found in the source after filtering and normalization.
- `flagged_entities` are summary entities the verifier could not ground in the source after filtering. They are best treated as inspection hints, not proof of hallucination.
- The verifier now ignores common noise such as dates, percentages, numbered list markers, URLs, and reference scaffolding, but it remains heuristic and can still miss paraphrased attributions or over-flag rephrasings.

## Disposition Summary

- The first stabilization pass should stay minimal and target the contract mismatches, not the larger architecture work.
- The follow-up pass focused on live-runtime semantics: real-model wiring, verified abstractive output, meaningful extractive length behavior, and a less noisy grounding signal.
